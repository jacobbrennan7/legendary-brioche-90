    ...
    reader.onload = () => {
      try {
        ...
        const imported: FlightEntry[] = rows.slice(1).map(r => ({
          date: toISO(r[idx.date] or ""),
          total: parseNumber(r[idx.total] or "0"),
          pic: parseNumber(r[idx.pic] or "0"),
          solo: parseNumber(r[idx.solo] or "0"),
          dual: parseNumber(r[idx.dual] or "0"),
          night: parseNumber(r[idx.night] or "0"),
          xc: parseNumber(r[idx.xc] or "0"),
          ifr: parseNumber(r[idx.ifr] or "0"),
          approaches: parseNumber(r[idx.approaches] or "0"),
          landings: parseNumber(r[idx.landings] or "0"),
          nightLandings: parseNumber(r[idx.nightLandings] or "0"),
        })).filter(e => e.date);
